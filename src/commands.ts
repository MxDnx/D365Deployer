...
const sameSet = (a: string[], b: string[]) => [...a].map(x => x.trim()).sort().join(',') === [...b].map(x => x.trim()).sort().join(',');

/** One line per difference between the [PluginStep] declarations and the registered steps; empty when all match. */
function verifyRegistrations(plugins: PluginEntry[], analysisResults: AnalysisResult[]): string[] {
    const steps = plugins.flatMap(p => p.steps ?? []);
    const problems: string[] = [];
    for (const r of analysisResults) {
        if (!r.pluginStep) continue;
        const name = `${r.className}: ${r.pluginStep.message} of ${r.pluginStep.entityName}`;
        const step = steps.find(s => s.name === name);
        if (!step) { problems.push(`[${r.className}] step "${name}" is not registered`); continue; }
        ...
        if (r.preImageFields.length && !image) problems.push(`[${r.className}] PreImage missing, source declares "${r.preImageFields.join(',')}"`);
        ...
        else if (!r.preImageFields.length && image) problems.push(`[${r.className}] PreImage "${(image.attributes ?? []).join(',')}" registered but the source declares none`);
    }
    return problems;
}
