...
export const CUSTOMIZATIONS_TEMPLATE = `<?xml version="1.0" encoding="utf-8"?>
...
export const RELATIONSHIPS_TEMPLATE = `<?xml version="1.0" encoding="utf-8"?>
...
export function syncSolutionWebResources(stagingDir: string, wrNames: string[]): void {
...
function readSolutionVersion(solutionPath: string): string {
    const content = fs.readFileSync(solutionPath, 'utf8');
    const match = content.match(/<Version>([\d.]+)<\/Version>/);
    return match ? match[1] : '1.0.0.0';
}

function buildWebResourcesXml(uniqueNames: string[], version: string): string {
    if (uniqueNames.length === 0) { return '  <WebResources />'; }
...
function updateCustomizationsXml(filePath: string, uniqueNames: string[], version: string): void {
...
function updateSolutionRootComponents(filePath: string, uniqueNames: string[]): void {
